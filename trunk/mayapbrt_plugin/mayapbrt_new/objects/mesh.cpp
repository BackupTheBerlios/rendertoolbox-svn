...
namespace pbrt {
	...
	MStatus Mesh::TranslationMatrix(std::ostream& fout) const {
		...
		fout << "Translate " <<  translation.x << " " << translation.y << " " << translation.z << std::endl;
		
		MEulerRotation rotation = matrix.eulerRotation().reorder(MEulerRotation::kXYZ);
		fout << "Rotation " << rotation.x << " 1 0 0" << std::endl;
		fout << "Rotation " << rotation.y << " 0 1 0" << std::endl;
		fout << "Rotation " << rotation.z << " 0 0 1" << std::endl;
		
		double scale[3];
		matrix.getScale(scale, MSpace::kWorld);
		fout << "Scale " << scale[0] << " " << scale[1] << " " << scale[2] << std::endl;
