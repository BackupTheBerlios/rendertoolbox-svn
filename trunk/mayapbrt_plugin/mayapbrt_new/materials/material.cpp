...
namespace pbrt {
	...
	int Material::colorTexture(std::ostream &fout) const {
		MStatus status;
		int texcolor;
		...
		if (!dgIt.isDone()) {
			...
			texcolor = ++textureNum;
			fout << "Texture \"" << texcolor << "\" \"color\" \"imagemap\" \"string filename\" [\"" << textureName.asChar() << "\"]" << std::endl;
		} else {
	...
	int Material::bumpTexture(std::ostream& fout) const {
		MStatus status;
		int texbump = 0;
		...
		if (!dgIt.isDone()) {
			...
			texbump = ++textureNum;
			fout << "Texture \"" << texbump << "\" \"float\" \"imagemap\" \"string filename\" [\"" << textureName.asChar() << "\"]" << std::endl;
		}
		
		return texbump;
	}
	
	std::ostream& operator<< (std::ostream& fout, const Material& obj) {
		obj.Insert(fout);
		return fout;
	}
