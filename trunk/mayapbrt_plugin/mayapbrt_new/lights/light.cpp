...
namespace pbrt {
	...
	Light* Light::LightFactory(const MDagPath &dagPath) {
		...
		MFnSpotLight spotTest(dagPath, &status); cout << (status==MStatus::kSuccess) << std::endl;		
		if (status==MStatus::kSuccess) {
			return new SpotLight(dagPath);
		}
		
		MFnPointLight pointTest(dagPath, &status); cout << (status==MStatus::kSuccess) << std::endl;		
		if (status==MStatus::kSuccess) {
	...
	MStatus Light::TranslationMatrix(std::ostream& fout) const {
		...
		fout << "ConcatTransform [" << matrix.matrix[0][0] << " " << matrix.matrix[0][1] << " " << matrix.matrix[0][2] << " " << matrix.matrix[0][3] << " " <<
									  ...
									  matrix.matrix[3][0] << " " << matrix.matrix[3][1] << " " << matrix.matrix[3][2] << " " << matrix.matrix[3][3] << "]" << std::endl;
