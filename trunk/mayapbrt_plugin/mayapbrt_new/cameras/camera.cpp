...
namespace pbrt {
	...
	Camera::Camera(MString &_imageName) throw (ExportError) {
	...
	void Camera::InsertPerspective(std::ostream& fout) {
		...
		fout << "\"float hither\" [" << camera.nearClippingPlane() << "]" << std::endl;
		
		fout << "Film \"tga\" \"integer xresolution\" [" << 640 << "] \"integer yresolution\" [" << 480 << "] \"string filename\" [\"" << imageName.asChar() << "\"]" << std::endl;
	}
	
	// overloaded output operator
	std::ostream& operator<< (std::ostream& fout, Camera& camera) {
